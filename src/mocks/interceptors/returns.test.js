...
describe('mocks/interceptors/returns', () => {
  it('GET /api/v2/returns/ retorna la lista con results y count', () => {
    const r = interceptReturns('/api/v2/returns/', { method: 'GET' });
    expect(r.status).toBe(200);
    expect(Array.isArray(r.data.results)).toBe(true);
    expect(r.data.count).toBe(r.data.results.length);
  });

  it('GET /api/v2/returns/<id>/ devuelve el detalle del comprador', () => {
    const r = interceptReturns('/api/v2/returns/1/', { method: 'GET' });
    expect(r.status).toBe(200);
  ...
  it('GET inexistente retorna 404', () => {
    const r = interceptReturns('/api/v2/returns/999/', { method: 'GET' });
    expect(r.status).toBe(404);
    expect(r.data.detail).toMatch(/no encontrada/i);
  });

  it('POST /api/v2/returns/ crea (201) con order_id + reason', () => {
    const r = interceptReturns('/api/v2/returns/', {
      method: 'POST',
  ...
  it('GET /api/v2/admin/returns/ incluye metrics + results', () => {
    const r = interceptReturns('/api/v2/admin/returns/', { method: 'GET' });
    expect(r.status).toBe(200);
  ...
  it('POST approve actualiza el status del item', () => {
    const r = interceptReturns('/api/v2/admin/returns/1/approve/', {
      method: 'POST',
    ...
    const after = interceptReturns('/api/v2/admin/returns/1/', { method: 'GET' });
    expect(after.data.status).toBe('APPROVED');
  });

  it('POST reject sin justification devuelve 400', () => {
    const r = interceptReturns('/api/v2/admin/returns/1/reject/', {
      method: 'POST',
      body:   JSON.stringify({}),
    });
    expect(r.status).toBe(400);
  ...
  it('POST refund antes de RECIBIDA devuelve 409', () => {
    const r = interceptReturns('/api/v2/admin/returns/1/refund/', {
      method: 'POST',
  ...
  it('POST reception -> refund flujo completo', () => {
    interceptReturns('/api/v2/admin/returns/1/reception/', {
      method: 'POST',
