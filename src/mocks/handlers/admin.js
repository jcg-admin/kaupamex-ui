...
export const adminHandlers = [
  ...
  http.get(`${BASE}/api/v1/admin/products/:id/`, ({ params }) => {
  ...
  http.patch(`${BASE}/api/v1/admin/products/:id/`, async ({ params, request }) => {
    const body = await request.json();
    const p = mockProducts.find(x => String(x.id) === params.id);
    if (!p) return new HttpResponse(null, { status: 404 });
  ...
  http.post(`${BASE}/api/v1/admin/products/:id/deactivate/`, ({ params }) => {
    const p = mockProducts.find(x => String(x.id) === params.id);
    if (!p) return new HttpResponse(null, { status: 404 });
    return HttpResponse.json({ ...p, is_active: false });
  }),
  http.get(`${BASE}/api/v1/admin/products/:id/variants/`, () =>
    HttpResponse.json({ count: 1, results: [{ id: 10, name: 'Default', stock: 12 }] }),
  ),
  http.post(`${BASE}/api/v1/admin/products/:id/variants/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ id: 99, ...body }, { status: 201 });
  }),
  http.patch(`${BASE}/api/v1/admin/products/:productId/variants/:variantId/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ id: 10, ...body });
  }),
  http.patch(`${BASE}/api/v1/admin/variants/:id/price/`, async ({ request }) => {
    const body = await request.json();
  ...
  http.patch(`${BASE}/api/v1/admin/categories/:id/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  http.post(`${BASE}/api/v1/admin/categories/:id/deactivate/`, ({ params }) => {
    const c = mockCategories.find(x => String(x.id) === params.id);
    if (!c) return new HttpResponse(null, { status: 404 });
  ...
  http.post(`${BASE}/api/v1/admin/orders/:id/status/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  http.post(`${BASE}/api/v1/admin/orders/:id/cancel/`, ({ params }) =>
  ...
  http.get(`${BASE}/api/v1/admin/inventory/variants/:id/movements/`, ({ params }) =>
    HttpResponse.json({
      count: 2,
      results: [
  ...
  http.patch(`${BASE}/api/v1/admin/product-discounts/:id/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  http.delete(`${BASE}/api/v1/admin/product-discounts/:id/`, () =>
    new HttpResponse(null, { status: 204 }),
  ),
  ...
  http.get(`${BASE}/api/v1/admin/roles/:role/permissions/`, ({ params }) =>
    HttpResponse.json({ role: params.role, permissions: mockPermissions[params.role] || [] }),
  ),
  http.put(`${BASE}/api/v1/admin/roles/:role/permissions/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  http.patch(`${BASE}/api/v1/admin/returns/:id/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  // Newsletter
  http.get(`${BASE}/api/v1/admin/newsletter/campaigns/`, () =>
  ...
  http.post(`${BASE}/api/v1/admin/newsletter/subscribers/:id/unsubscribe/`, ({ params }) =>
  ...
  // Logistics
  http.post(`${BASE}/api/v1/logistics/guides/:id/confirm-delivery/`, ({ params }) =>
