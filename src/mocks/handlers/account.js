...
export const accountHandlers = [
  // Profile
  http.get(`${BASE}/api/v2/account/profile/`, () =>
    HttpResponse.json(mockProfile),
  ),
  http.patch(`${BASE}/api/v2/account/profile/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ ...mockProfile, ...body });
  }),

  // Avatar
  http.post(`${BASE}/api/v2/account/profile/avatar/`, () =>
    HttpResponse.json({ avatar: '/media/avatars/usuario1.jpg' }),
  ),
  http.delete(`${BASE}/api/v2/account/profile/avatar/`, () =>
    new HttpResponse(null, { status: 204 }),
  ),

  // Change password
  http.post(`${BASE}/api/v2/account/change-password/`, async ({ request }) => {
    const body = await request.json();
  ...
  // Email verification
  http.post(`${BASE}/api/v2/account/verify-email/`, async ({ request }) => {
    const body = await request.json();
  ...
  http.post(`${BASE}/api/v2/account/resend-verification/`, () =>
    HttpResponse.json({ detail: 'Email de verificación enviado' }),
  ),

  // Two-factor auth
  http.get(`${BASE}/api/v2/account/2fa/status/`, () =>
    HttpResponse.json({ enabled: false, method: null }),
  ),
  http.post(`${BASE}/api/v2/account/2fa/enable/`, () =>
    HttpResponse.json({ secret: 'JBSWY3DPEHPK3PXP', qr_url: 'data:image/png;base64,abc' }),
  ),
  http.post(`${BASE}/api/v2/account/2fa/verify/`, async ({ request }) => {
    const body = await request.json();
    if (body.code === '123456') {
  ...
  http.post(`${BASE}/api/v2/account/2fa/disable/`, () =>
    HttpResponse.json({ detail: '2FA desactivado' }),
  ),

  // Addresses
  http.get(`${BASE}/api/v2/account/addresses/`, () =>
    HttpResponse.json({ count: mockAddresses.length, results: mockAddresses }),
  ),
  http.post(`${BASE}/api/v2/account/addresses/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ id: mockAddresses.length + 1, is_default: false, ...body }, { status: 201 });
  }),
  http.patch(`${BASE}/api/v2/account/addresses/:id/`, async ({ params, request }) => {
    const body = await request.json();
  ...
  http.delete(`${BASE}/api/v2/account/addresses/:id/`, () =>
    new HttpResponse(null, { status: 204 }),
  ),
  http.post(`${BASE}/api/v2/account/addresses/:id/set-default/`, ({ params }) => {
    const addr = mockAddresses.find(a => String(a.id) === params.id);
    if (!addr) return new HttpResponse(null, { status: 404 });
  ...
  http.get(`${BASE}/api/v2/account/notifications/preferences/`, () =>
    HttpResponse.json(mockNotificationPrefs),
  ),
  http.patch(`${BASE}/api/v2/account/notifications/preferences/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ ...mockNotificationPrefs, ...body });
  }),

  // Account deactivation
  http.post(`${BASE}/api/v2/account/deactivate/`, async ({ request }) => {
    const body = await request.json();
    if (!body.password) {
  ...
  http.get(`${BASE}/api/v2/account/orders/`, () =>
    HttpResponse.json({
      count: 2,
      results: [
  ...
  http.get(`${BASE}/api/v2/account/orders/:id/`, ({ params }) =>
    HttpResponse.json({
      id: Number(params.id),
  ...
  http.get(`${BASE}/api/v2/account/returns/`, () =>
    HttpResponse.json({ count: 0, results: [] }),
  ),
  http.post(`${BASE}/api/v2/account/returns/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ id: 1, status: 'PENDING', ...body }, { status: 201 });
  }),

  // Wishlist
  http.get(`${BASE}/api/v2/account/wishlist/`, () =>
    HttpResponse.json({ count: 0, results: [] }),
  ),
  http.post(`${BASE}/api/v2/account/wishlist/`, async ({ request }) => {
    const body = await request.json();
    return HttpResponse.json({ id: 1, product_id: body.product_id }, { status: 201 });
  }),
  http.delete(`${BASE}/api/v2/account/wishlist/:id/`, () =>
    new HttpResponse(null, { status: 204 }),
  ),
];
