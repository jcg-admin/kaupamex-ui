 ...
 * envia en siguientes requests a /api/v1/cart/.
 */
...
describe('apiService X-Cart-Token (DEC-BC-07 / B-07)', () => {
  ...
  it('extrae X-Cart-Token del response y lo guarda en _cartToken', async () => {
    ...
    const res = await api.post('/api/v1/cart/items/', { product_id: 1 });

    expect(res.status).toBe(200);
    expect(api.getCartToken()).toBe('abc-12345');
  });

  it('envia X-Cart-Token en siguientes requests a /api/v1/cart/', async () => {
  ...
  it('NO envia X-Cart-Token a paths fuera de /api/v1/cart/', async () => {
  ...
  it('dos requests consecutivos a cart propagan el token desde el primer response', async () => {
    ...
    await api.post('/api/v1/cart/items/', { product_id: 1 });
    await api.get('/api/v1/cart/');
